...
8. PendingSelection 拥有 Gameplay-authored exact request identity，不复用 Presentation SelectionGeneration
...
12. Damage historical reducer/validator 的唯一完整语义
13. Hit / Enemy Attack combatant animation cue 与 detached Damage 的关系
14. G8-C legacy blocking duration 的唯一 timing authority
15. G8-C 使用不可被其它 Blocking Presentation 抵消的 compatibility debt
16. ordinary Skip 按旧 Skip 语义清除 staging compatibility debt，但保持 SessionToken
17. 首版 DamageNumber 由 NativeTick + finite VisualDuration 结束，不额外保留不可达 HardTimeout
```
...
- 当前 `FPendingCardSelectionReadView` 没有 Gameplay request generation；现有 `SelectionGeneration` 属于 SelectionArea presentation ownership，并且可能直到第一次点击候选牌时才建立，因此不能作为 PendingSelection readiness 的 request identity。
...
理由：这些事件改变的是 chronological playback state，不是 Presentation authority identity。
...
→ mint/establish new session（若仍为 PresentationOwned 模式）
...
### 8.4 PendingSelection：新增 Gameplay exact request identity

Pending card selection 已经是 Gameplay-authoritative lifecycle，因此必须拥有 Gameplay-authored request identity。首版新增用途明确的小型 token：

```text
PendingSelectionRequestToken =
  BattleId
  + PendingSelectionRequestGeneration
```
...
```text
PendingSelectionRequestGeneration
- 0 / invalid value 不代表 active request
- 每次 USelectionResolver::BeginSelection() 成功建立新的 pending request 时 mint
- 同一 BattleId 内单调递增且不得复用
- request resolve / legal cancel / fault cleanup 后该 token 失效
- 下一 pending request 必须得到不同 generation
- 若 SelectionResolver 可能在同一 BattleId 下 replacement，generation mint source 必须存活于 replacement 或通过等价 epoch 防 ABA
```

推荐由 Battle-scoped authority 持有 monotonic counter，由 `USelectionResolver` 在 `BeginSelection()` 成功时取得；如果可以证明 Resolver 在 BattleId 生命周期内不会 replacement，也可由 Resolver 持有，但“不跨 replacement 复用”是硬合同。

`FPendingCardSelectionReadView` 必须暴露 exact `PendingSelectionRequestGeneration`，使 ViewModel/HUD 能区分：

```text
旧 pending request 的 read/callback
!=
相同 SelectionSource / 相同候选集的下一 pending request
```

现有 Presentation `SelectionGeneration` **不等于**这个 request generation：

```text
PendingSelectionRequestGeneration
→ Gameplay pending request identity
→ request 创建时就存在
...
因此 readiness 不得要求“尚未创建的 SelectionGeneration”来允许第一次 pending-selection click。Presentation lifecycle 一旦建立，后续 visual transition / submit 仍继续校验已有 `SelectionGeneration`，但它是附加 presentation guard，不替代 Gameplay request token。
...
PendingCardSelection:
  SessionToken + PendingSelectionRequestToken
...
新 Battle、ControllerEpoch replacement、PresentationOwned session replacement、authority source replacement、revision replacement、PendingSelectionRequestGeneration replacement、selected-card replacement都会使对应旧 credential 失效。普通 active-envelope reconcile 不替换 SessionToken，但其 displayed revision/read surface 变化仍会让不匹配的旧 credential stale。
...
EvaluateInteractionReadiness(CurrentMode, AuthoritySource)
  ...
  |
  +-- PendingCardSelection
        → exact PendingSelectionRequestToken / boundary / read guard
...
旧点击不得被带入更新的 Gameplay state，也不得在新 ControllerEpoch、新 PendingSelectionRequestGeneration 或新 target surface 重放。
...
A12. sanity ceiling pre-commit decline
A13. PlayCommittedDamageCombatantCues API boundary
A14. post-publication stale rejection harness
```
...
- PendingSelectionRequestToken 已由 Gameplay authority mint 并暴露到 read view
...
```text
PendingSelectionRequestGeneration exact identity
...
back-to-back PendingSelection with same source/candidates but different PendingSelectionRequestGeneration
...
| Pending request exact identity | 每个 Gameplay pending request 在创建时就有非零 exact generation；back-to-back 同内容 request 仍不同 token |
...
| G8-C Skip debt | ordinary Skip 清除被 collapse chronology 的 compatibility debt，但保持 SessionToken |
...
连续两个内容相同的 pending request 具有不同 PendingSelectionRequestGeneration，旧 callback 不可重放
...
记录默认 DamageNumber `VisualDuration`、实际 input-ready 时间、legacy Blocking Damage duration 来源、G8-C accumulated debt 和后续 Blocking card-tail duration。
...
G8-C 每个 detached Damage 只增加 CompatibilityDebtSeconds，不使用会被其它 Blocking 时间抵消的 last-deadline 算法
...
Gameplay pending request 在 BeginSelection 成功时获得 exact PendingSelectionRequestGeneration
PendingSelectionRequestGeneration 与 Presentation SelectionGeneration 类型/职责分离，第一次 click 不形成循环依赖
