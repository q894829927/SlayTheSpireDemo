...
bool FPhase6UIA2D5StatusLifecycleTest::RunTest(const FString& Parameters)
{
	...
	if (!TestTrue(TEXT("Acceptance fixture starts"), Fixture.Start({})))
	{
		return false;
	}
	...
	// 0 -> 2 Applied. Keep a stale exact-instance Action alive from this point.
	...
	UStatusInstance* WeakA = Container->FindStatusById(TEXT("Weak"));
	...
	TStrongObjectPtr<UReduceStatusAction> StaleReduce(
		NewObject<UReduceStatusAction>(Queue)
	);
	...
	TestNull(TEXT("Displayed status remains absent before Applied completion"), FindDisplayedStatus(Fixture.ViewModel, TEXT("Weak")));
	...
	// 2 -> 3 Increased against the same concrete instance.
	...
	// TurnEndDecay 2 -> 1 through the real end-turn macro flow. This may publish
	// additional non-status Records/Envelopes; A2D5 validates them per Envelope.
	...
	TestNull(TEXT("Displayed Weak removed after playback completion"), FindDisplayedStatus(Fixture.ViewModel, TEXT("Weak")));

	// Recreate same StatusId as a new concrete runtime identity: Weak#B 0 -> 2.
	...
	UStatusInstance* WeakB = Container->FindStatusById(TEXT("Weak"));
	...
	// Execute the retained Weak#A action only now. It receives the current formal
	// Resolution writer; it never reuses the writer from the Resolution that
	// created Weak#A. Exact-instance membership makes this a Gameplay NoOp.
	...
	// Validate every captured Resolution independently. Do not flatten them into
	// one synthetic reducer history.
	TestTrue(
	...
	// Compare the actual committed status history in producer order. Never sort it.
	...
	// The stale action must not append a seventh StatusChanged record.
