...
namespace
{
	...
	FText ResolveAutomaticDescription(const UCardInstance* Card, ACombatant* Source,
		bool bRich, const TArray<FImmediatePreviewOperation>* Operations = nullptr)
	{
		FCardEffectPreviewContext Context;
		Context.Card = Card;
		Context.Source = Source;
		...
		{
			const UCardEffect* Effect = Effects[Index].Get();
			if (!IsValid(Effect))
			{
				continue;
			}
			FPreviewTextArgumentBuilder Builder;
			Effect->BuildAutoDescriptionArguments(Context, Builder);
			TArray<FImmediatePreviewOperation> EffectOperations;
			if (Operations)
			{
				for (const FImmediatePreviewOperation& Operation : *Operations)
				{
					// Names are local to one Effect; never overwrite another instance.
					if (Operation.EffectIndex == Index)
					{
						Builder.OverrideInteger(Operation.SemanticArgumentName, Operation.ResolvedAmount);
						EffectOperations.Add(Operation);
					}
				}
			}
			TMap<FName, FText> RichOverrides;
			BuildRichPreviewOverrides(EffectOperations, RichOverrides);
			const FText Line = FormatDescription(Effect->GetAutoDescriptionFormat(Context), Builder,
				Card->GetDebugLabel(), bRich, bRich ? &RichOverrides : nullptr);
			if (!Line.IsEmpty())
			{
				Lines.Add(Line);
			}
...
FText FBattleTextResolver::ResolveCardDescription(const UCardInstance* Card, ACombatant* Source)
{
	...
	if (Card->GetDefinition()->bAutoGenerateDescription)
	{
		return ResolveAutomaticDescription(Card, Source, false);
	}
	FPreviewTextArgumentBuilder Builder;
...
FText FBattleTextResolver::ResolveCardRichDescription(const UCardInstance* Card, ACombatant* Source)
{
	...
	if (Card->GetDefinition()->bAutoGenerateDescription)
	{
		return ResolveAutomaticDescription(Card, Source, true);
	}
	FPreviewTextArgumentBuilder Builder;
	if (!BuildCardDescriptionArguments(Card, Source, Builder))
	{
	...
	const TArray<FImmediatePreviewOperation>& Operations)
{
	...
	if (Card->GetDefinition()->bAutoGenerateDescription)
	{
		return ResolveAutomaticDescription(Card, Source, false, &Operations);
	}
	FPreviewTextArgumentBuilder Builder;
	if (!BuildCardDescriptionArguments(Card, Source, Builder))
	{
	...
	const TArray<FImmediatePreviewOperation>& Operations)
{
	...
	if (Card->GetDefinition()->bAutoGenerateDescription)
	{
		return ResolveAutomaticDescription(Card, Source, true, &Operations);
	}
	FPreviewTextArgumentBuilder Builder;
	if (!BuildCardDescriptionArguments(Card, Source, Builder))
	{
...
bool FBattleTextResolver::ValidateCardDefinition(const UCardData* Definition, TArray<FText>& OutErrors)
{
	...
	if (Definition->bAutoGenerateDescription)
	{
		for (const TObjectPtr<UCardEffect>& Effect : Definition->Effects)
		{
			if (!IsValid(Effect))
			{
				AddValidationError(OutErrors, FString::Printf(TEXT("Card %s contains an invalid Effect."), *CardName));
				continue;
			}
			Effect->ValidatePreviewConfiguration(OutErrors);
			FCardEffectPreviewContext Context;
			if (Effect->GetAutoDescriptionFormat(Context).IsEmpty())
			{
				AddValidationError(OutErrors, FString::Printf(TEXT("Effect %s must supply an automatic description."), *Effect->GetClass()->GetName()));
			}
		}
